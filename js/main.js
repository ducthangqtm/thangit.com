...
/* ==========================================================================
   10. LIVE ANIMATED FAVICON (Canvas-driven, Cross-Browser: Chrome, Edge, Firefox)
   ========================================================================== */
function initAnimatedFavicon() {
  ...
  function drawBadgePath(context, x, y, w, h, r) {
    context.beginPath();
  ...
  function renderFavicon() {
    ...
    // 1. Dark Cyber Badge Base
    drawBadgePath(ctx, 1, 1, 30, 30, 7);
    ...
    // 2. Scanning Perimeter Beam (rotating around the border)
    ctx.save();
    const scanProgress = (frame % 50) / 50;
    drawBadgePath(ctx, 1, 1, 30, 30, 7);
    ctx.lineDashOffset = -scanProgress * 115;
    ctx.setLineDash([16, 44]);
    ctx.strokeStyle = '#00f0ff';
    ctx.lineWidth = 1.6;
    ctx.stroke();
    ctx.restore();

    // 3. Center 'i' Radar Wave Pulse
    const radarPhase = (frame % 25) / 25;
    const radarRadius = 1.5 + radarPhase * 6.5;
    const radarAlpha = Math.max(0, 1 - radarPhase);
    ctx.save();
    ctx.beginPath();
    ctx.arc(16, 8.5, radarRadius, 0, Math.PI * 2);
    ctx.strokeStyle = `rgba(0, 255, 157, ${radarAlpha.toFixed(2)})`;
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(16, 8.5, 1.9, 0, Math.PI * 2);
    ctx.fillStyle = beaconPulse > 0.4 ? '#00ff9d' : '#00f0ff';
    ctx.shadowColor = '#00ff9d';
    ctx.shadowBlur = 4;
    ctx.fill();
    ctx.restore();

    // 4. Soft Rounded Letters "T - i - T"
    ctx.save();
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.lineWidth = 2.6;
  ...
  function start() {
    if (!animTimer) {
      animTimer = setInterval(renderFavicon, 90); // ~11 fps
    }
  }
  ...
  // Energy & Performance: Pause when tab is inactive
